...
if [[ ${LENGTH_HOSTNAME} -eq "1" ]] ; then
    echo "Single-character hostnames are not permitted in WEKA clusters.  Please rename host ${SHORT_HOSTNAME}"
    RETURN_CODE=1
    exit ${RETURN_CODE}
fi
...
if [[ $? -eq 0 ]]; then
    echo "Hostnames must conform to RFC 952"
    echo "The hostname ${SHORT_HOSTNAME} appears to contain a character other than [A-Z], [a-z], -, and [0-9]."
