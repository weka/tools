...
for ROLE in COMPUTE DRIVES FRONTEND; do
    if [[ $(weka cluster process -b -F role="${ROLE}" -o netmode --no-header | sort | uniq | wc -l) -gt 1 ]]; then
        RETURN_CODE=254
    ...
    # Check for comptability -- UDP fallback did not exist in earlier versions
    if weka debug net ports 0 -F backendType="DPDK" &> /dev/null; then
