...
def parse_hangingios_description(description):
    pattern = re.compile(
        r'HangingIos\(NodeId<\d+>, (?:(\d+) days?, )?(?:(\d+) hours?, )?(?:(\d+) minutes?, )?(?:(\d+) secs?, and )?(?:\d+ ms, )?fid=0x[a-fA-F0-9]+, entry=\d+, operation=WEKAFS_SET_FILE_ACCESS, state=AcquiringFileLease, SnapViewId<(\d+)>, Pid<\d+>, InodeId<(\d+)>\)'
    )
...
def main():
    ...
    if not weka_alerts:
        logging.info("No hangingio alerts found")
        sys.exit()

    hangingios_description = weka_alerts[0]['description']
    matches = parse_hangingios_description(hangingios_description)

    filtered_entries = filter_and_collect_entries(matches, args.age)

    if filtered_entries:
        logging.info(f"Found {len(filtered_entries)} hanging I/O entries exceeding {args.age} hours.")
        process_entries(filtered_entries)
    else:
        logging.info(f"No hanging I/O entries found exceeding {args.age} hours.")
