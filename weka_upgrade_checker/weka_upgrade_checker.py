    ...
    def __init__(self, host_json):
        ...
        self.aws_id = (
            str(host_json["aws"]["instance_id"]
                ) if "instance_id" in host_json else None
        )
        self.aws_type = (
...
def get_online_version():
    ...
    try:
        file_content = subprocess.check_output(
            curl_command, shell=True, text=True)
...
def weka_cluster_checks():
        ...
        if V("4.0") <= V(weka_version) < V("4.1"):
            try:
                for w_cl_machine in weka_cl_machines:
                    ...
                    if ".".join(clsw_version[:2]) != check_version:
                        cl_machine_need_upgrade += [
                            w_cl_machine.name, w_cl_machine.ip]
            ...
            try:
                for w_cl_server in weka_cl_servers:
                    ...
                    if ".".join(clsw_version[:2]) != check_version:
                        cl_machine_need_upgrade += [
                            w_cl_server.name, w_cl_server.ip]
            except NameError as e:
                WARN("⚠️  Unable to determine client weka version")
        else:
            try:
    ...
    if V(weka_version) >= V("4.0"):
        ...
        if not any(
            check_duplicate_core_ids(
                containers) or validate_core_ids(containers)
    ...
    if V(weka_version) > V("3.10"):
        if len(nfs_server_hosts) != 0:
            ...
            if nfs_server_hosts[0]["status"] == "OK":
                ...
                WARN(
                    f'{" " * 5}⚠️  NFS Server {nfs_server_hosts[0]["name"]} Not OK')
...
def protocol_host(backend_hosts, s3_enabled):
    ...
    s3_enabled = json.loads(subprocess.check_output(
        ["weka", "s3", "cluster", "-J"]))
    if s3_enabled:
        weka_s3 = json.loads(
            subprocess.check_output(["weka", "s3", "cluster", "status", "-J"])
...
def backend_host_checks(
    ...
    for host_name, result in results:
        if result is None:
            WARN(
                f"Unable to determine Host: {host_name} available trace space")
        else:
    ...
    if V(weka_version) >= V("4.2"):
        ...
        for host_name, result in results:
            if result is None:
                WARN(
                    f"Unable to determine Host: {host_name} cpu instruction set")
            else:
